...
//Begin Main Script
	...
	if ($user) {
		...
		if (!empty($user_profile )) {
			...
			$sql = "SELECT facebookid FROM users WHERE facebookid='" . $fbuid . "'";
			$result = $conn->query($sql);

			if ($result->num_rows > 0) {
				while($row = $result->fetch_assoc()) {
					//They are a user! - Make them a token
					generatetoken('', $row["facebookid"]);
				}
			}
			else die('Sorry - You do not have permission to use this service. Please send the number ' . $fbuid . ' to James Bithell, ideally using Facebook messenger');
		}
	else {
